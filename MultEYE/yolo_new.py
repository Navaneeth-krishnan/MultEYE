...
class MultEYE(nn.Module):
  def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):
    ...
    self.backbone = ModelB()
    self.head = ModelD()

  def forward(self, x):
    latentspace = self.backbone(x)
    detect = self.head(latentspace)
    return detect#, output2
...
def parse_modelb(d, ch):  # model_dict, input_channels(3)
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5)
    ...
    for i, (f, n, m, args) in enumerate(d['head']):  # from, number, module, args
        ...
        layers.append(m_)
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save),layers, save, c2

def parse_modeld(d, ch):  # model_dict, input_channels(3)
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5)
